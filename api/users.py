...
def create_user_with_plan(payload: UserCreateRequest, db: Session = Depends(get_db_write)):
    """Create a new user and generate an initial daily meal plan.

    This endpoint computes nutrition targets using the `NutritionCalculator`,
    persists the new user, generates a plan with `RecommendationEngine`,
    saves the meal plan, and returns user + plan details.
    ...
    Returns:
        `UserWithMealPlanResponse` containing the user and generated plan.
    ...
    plan = recommendation_service.generate_daily_meal_plan(user, all_meals)

    meal_plan = models.MealPlan(
        user_id=user.id,
        plan_date=__import__('datetime').date.today(),
        breakfast_id=plan['breakfast']['id'],
        lunch_id=plan['lunch']['id'],
        dinner_id=plan['dinner']['id'],
        snack_id=plan['snack']['id'] if plan.get('snack') else None,
        total_calories=plan['daily_totals']['calories'],
        total_protein=plan['daily_totals']['protein'],
        total_carbs=plan['daily_totals']['carbs'],
        total_fat=plan['daily_totals']['fat'],
    )
    meal_plan = save(db, meal_plan)

    logger.info("User %s created with id=%s", user.name, user.id)
